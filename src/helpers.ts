...
export const inquirerInput = async <T>(
    msg: string,
    validate?: (val: T, answers?: T[]) => boolean | string | Promise<boolean | string>
) =>
    (
        await inquirer.prompt([
            {
...
export const inquirerPassword = async (
    msg: string,
    validate: (val: string, answers?: string[]) => boolean | string | Promise<boolean | string>
) =>
    (
        await inquirer.prompt([
            {
...
export const headLines = (...msg: string[]) => {
    ...
    const msgLength = sumBy(msg, m => m.replace(notAnsiRegex, '').length) + msg.length;
